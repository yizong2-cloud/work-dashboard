...
> 当前实现已覆盖：结构化导入与严格校验、草稿/开放/关闭生命周期、匿名可选身份填答、独立答卷收件箱、Markdown/JSON 导出、澄清记录，以及 Supabase/本地离线存储。后续修改应以现有代码与测试为基线，不要按“待实现”重新搭建。
...
5. 已关闭表单只读，不能再提交。
6. 发起人列表提供明确但不虚假的下一步：无答卷的开放表单为“待回复”；已有答卷的开放表单为“待整理给 Agent”；已关闭表单为“已收口”。“待整理”只表示可导出，页面不臆测结果是否已被复制给外部 Agent。
...
按本次确认，v1 沿用 Workboard 的内部协作模式：**不登录、不做严格角色限制**。持有表单链接的内部成员可以查看并填写；导出页面也不额外加门槛。这个取舍以“分享与填写效率优先”为准，不能为了形式上的权限完善而要求内部成员反复登录或验证。
