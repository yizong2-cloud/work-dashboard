...
>
> **重要**：用户说「开始更新」时，只读取并执行 **`workflow/dashboard-update.skill.md`**。
> 它会在健康预检通过后再要求读取 `docs/KNOWLEDGE_BASE.md` 和紧凑审查包；不要把本手册、
> `docs/UPDATE_WORKFLOW.md`、`docs/WORKFLOW_OVERVIEW.md` 一起塞进日常上下文。完整文档只在
> 工作流维护、排障或需要查字段/命令细节时展开，避免既浪费上下文又出现规则分叉。
