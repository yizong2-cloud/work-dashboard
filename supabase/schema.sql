...
-- Agent 处理箱专用原子回写：回复正文和线程状态必须同时落库，避免
-- CLI 在网络中断时留下“已经解释但仍显示待处理”的半完成状态。
...
create table if not exists public.notification_outbox (
  ...
  event_type  text not null check (event_type in
              ('task_update','task_update_progress','task_nudged','feedback_created','feedback_replied','feedback_resolved','decision_response_submitted')),
...
begin
  ...
  alter table public.notification_outbox add constraint notification_outbox_event_type_check check
    (event_type in ('task_update','task_update_progress','task_nudged','feedback_created','feedback_replied','feedback_resolved','decision_response_submitted'));
end $$;
...
-- ---------------- 11. 决策答卷通知分流 ----------------
-- 答卷属于 Leader 私有收件信息：进入个人机器人，不回群。
